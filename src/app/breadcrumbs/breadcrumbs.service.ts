...
export class BreadcrumbsService {
  ...
  private resolveBreadcrumbs(route: ActivatedRoute): Observable<Breadcrumb[]> {
    ...
    if (
      ...
      let metadataBK = key['metadata'];

      if (metadataBK && metadataBK['dc.language']) {

        const dcLanguage: string = metadataBK['dc.language'][0]['value'];
        let language: string;
        ...
        for (let key in metadataBK) {
    
          metadata = metadataBK[key].filter(item => {
            return item['language'] == null || item['language'] == language;
          });
    
          if (metadata.length > 0) {
            metadataBK = {...metadataBK, [key]: [ ...metadata ]};
          }
        }
    
        key['metadata'] = metadataBK;
      }
