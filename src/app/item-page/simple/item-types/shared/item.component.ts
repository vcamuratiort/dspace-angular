...
export class ItemComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    let objectBK = this.object['metadata'];

    if (objectBK['dc.language']) {

      const dcLanguage: string = objectBK['dc.language'][0]['value'];
      let language: string;
      ...
      for (let key in objectBK) {
  
        metadata = objectBK[key].filter(item => {
          return item['language'] == null || item['language'] == language;
        });
  
        if (metadata.length > 0) {
          objectBK = {...objectBK, [key]: [ ...metadata ]};
        }
      }
  
      this.object['metadata'] = objectBK;
    }
